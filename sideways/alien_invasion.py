...
class AlienInvasion:
	...
	def _create_alien(self, alien_number, row_number):
		...
		alien.rect.x = (self.settings.screen_width) - alien.rect.width - 2 * alien.rect.width * row_number
		self.aliens.add(alien)

	def _update_aliens(self):
		"""Check if the fleet is at an edge the update the 
		   positions of all the fleet"""
		self._check_fleet_edges()
		self.aliens.update()
		#look for collisions alien-ship
	...
	def _change_fleet_direction(self):
		"""Drop the entrie fleet and change it's direction"""
		for alien in self.aliens.sprites():
